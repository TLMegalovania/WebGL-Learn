...
function initGL() {
    ...
    console.log(rotver);
    const gl = document.getElementById("glcanvas")?.getContext("webgl2"), vsSource = document.getElementById("vertex-shader")?.textContent, fsSource = document.getElementById("fragment-shader")?.textContent;
    if (!gl) {
...
function divideTriangle(points, depth) {
    if (depth <= 0)
        return points.reduce((res, item) => {
            res.push(...item);
            return res;
        }, []);
    const mids = new Array(3);
